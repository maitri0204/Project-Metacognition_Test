...
import { ParentTestResult, Question, Answer } from "@/types";

// ── Section metadata (parent uses "Section" not "Domain") ───
...
> = {
  ...
  2: { name: "Planning",   shortName: "Planning",   color: "#3b82f6", bg: "bg-blue-50",   maxScore: 50 },
  ...
  return (
    <div className="flex flex-col items-center">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-md" aria-label="Metacognition Quadrant Graph">
        ...
        <circle cx={px} cy={py} r={8} fill="white" stroke="#2563eb" strokeWidth={3} />
        <circle cx={px} cy={py} r={4} fill="#2563eb" />
        <rect x={px - 42} y={py - 36} width={84} height={24} fill="#1d4ed8" rx={6} />
        <text x={px} y={py - 20} fill="white" fontSize={11} fontWeight={700} textAnchor="middle">({Math.round(kp)}%, {Math.round(rp)}%)</text>
      </svg>
      <p className="mt-2 text-sm font-semibold text-gray-700">
        Quadrant:{" "}
        <span className={kp >= 50 && rp >= 50 ? "text-green-700" : kp < 50 && rp >= 50 ? "text-blue-700" : kp < 50 && rp < 50 ? "text-red-700" : "text-yellow-700"}>
          {quadrantLabel}
        </span>
      </p>
    </div>
...
export default function ParentResultDetailPage() {
  ...
  if (!result) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={() => router.push("/student/parent-results")} className="mt-4 text-blue-600 underline">
          Back to Parent Results
        </button>
      </div>
    );
  }
  ...
  // Build populated questions grouped by section from answers
  ...
  Object.values(sectionGroups).forEach((arr) =>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      {/* Back */}
      <Link href="/student/parent-results" className="inline-flex items-center gap-2 text-blue-600 hover:underline text-sm font-medium">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div>
            ...
            <p className="text-sm font-medium text-gray-900 mt-0.5">{p.firstName} {p.middleName ? `${p.middleName} ` : ""}{p.lastName}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Relation</p>
            <p className="text-sm font-medium text-gray-900 mt-0.5">{p.relation}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Email</p>
            <p className="text-sm font-medium text-gray-900 mt-0.5">{p.email}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Mobile</p>
            <p className="text-sm font-medium text-gray-900 mt-0.5">{p.mobile}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-500 mb-6">
          X-axis: Knowledge (Awareness - Section 1) &nbsp;|&nbsp; Y-axis: Regulation (Sections 2-5)
        </p>
          ...
          <>
              ...
              <nav className="flex overflow-x-auto">
                {[1, 2, 3, 4, 5].map((sn) => {
                  ...
                  return (
                    ...
                    >
                      {isActive && (
                        <span className="absolute bottom-0 left-0 right-0 h-[2.5px] rounded-t-full" style={{ backgroundColor: info.color }} />
                      )}
                      <span
            ...
            {(() => {
              ...
              if (!items || items.length === 0) {
                return (
                  <div className="p-6">
                    <p className="text-gray-400 text-sm text-center py-8">No answers for this section.</p>
                  </div>
                );
              }
                ...
                <div className="p-6">
                  ...
                  >
                    <div className="flex-1">
                      <p className="font-bold text-gray-900">Section {sn}: {info.name}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold" style={{ color: info.color }}>{sectionScore}/{info.maxScore}</p>
                      <p className="text-xs text-gray-500">{sPct}%</p>
                    </div>
                  </div>
                    ...
                    {items.map(({ question: q, selectedOption }) => (
                      <div key={q._id} className="rounded-xl border border-gray-100 p-4 bg-gray-50">
                        <p className="font-medium text-gray-900 mb-3">
                          <span className="font-bold mr-1" style={{ color: info.color }}>Q{q.questionNumber}.</span>
                          {q.questionText}
                        </p>
