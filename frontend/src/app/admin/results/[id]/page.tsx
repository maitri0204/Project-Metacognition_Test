...
import { TestResult, Question, Answer, User } from "@/types";
...
function QuadrantGraph({
  ...
  if (kp >= 50 && rp >= 50) {
    ...
    hlColor = "rgba(34,197,94,0.25)"; quadrantLabel = "Expert Learner";
  } else if (kp < 50 && rp >= 50) {
    hlX = px; hlY = py; hlW = midX - px; hlH = midY - py;
    hlColor = "rgba(59,130,246,0.25)"; quadrantLabel = "Reflective but Unstructured Learner";
  } else if (kp < 50 && rp < 50) {
    hlX = px; hlY = midY; hlW = midX - px; hlH = py - midY;
    hlColor = "rgba(239,68,68,0.25)"; quadrantLabel = "Unaware Learner";
  } else {
    hlX = midX; hlY = midY; hlW = px - midX; hlH = py - midY;
    hlColor = "rgba(234,179,8,0.25)"; quadrantLabel = "Strategic Learner";
  }
  ...
  return (
    <div className="flex flex-col items-center">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-md">
        ...
        {gridPcts.map((pct) => (
          <g key={pct}>
            <line x1={toSvgX(pct)} y1={PAD_T} x2={toSvgX(pct)} y2={PAD_T + plotH}
              stroke="#e5e7eb" strokeWidth={pct === 50 ? 1.5 : 1} strokeDasharray={pct === 50 ? "0" : "4,3"} />
            <line x1={PAD_L} y1={toSvgY(pct)} x2={PAD_L + plotW} y2={toSvgY(pct)}
              stroke="#e5e7eb" strokeWidth={pct === 50 ? 1.5 : 1} strokeDasharray={pct === 50 ? "0" : "4,3"} />
          </g>
        ))}
        {hlW > 0 && hlH > 0 && (
        ...
        <text x={14} y={PAD_T + plotH / 2} fill="#374151" fontSize={13} fontWeight={700} textAnchor="middle"
          transform={`rotate(-90, 14, ${PAD_T + plotH / 2})`}>Regulation (Planning + Monitoring + Regulation + Reflection)%</text>
        <circle cx={px} cy={py} r={8} fill="white" stroke="#1d4ed8" strokeWidth={3} />
        <circle cx={px} cy={py} r={4} fill="#1d4ed8" />
        <rect x={px - 42} y={py - 36} width={84} height={24} fill="#1e40af" rx={6} />
...
export default function AdminResultDetailPage() {
  ...
  if (!result) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={() => router.push("/admin/dashboard")} className="mt-4 text-blue-600 underline">
          Back to Dashboard
        </button>
      </div>
    );
  ...
  const student = result.student as User;
  ...
  // Build domain groups from populated answers
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="text-right">
            <p className="text-5xl font-bold text-blue-600">{result.totalScore}</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="space-y-4">
          {[1, 2, 3, 4, 5].map((dn) => {
            ...
            const score = ds[`domain${dn}` as keyof typeof ds];
            const pct = Math.round((score / info.maxScore) * 100);
            return (
              <div key={dn}>
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-sm font-semibold text-gray-700">D{dn}: {info.name}</span>
                  <span className="text-sm font-bold text-gray-900">{score}/{info.maxScore} ({pct}%)</span>
                </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 pt-6 pb-0">
          <h2 className="text-lg font-bold text-gray-900 mb-4">Student&apos;s Answers - Domain Wise</h2>
        </div>
